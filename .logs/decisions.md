...
## 2026-07-21 — Admin analytics dashboard: scope + architecture
Chosen (BRAINSTORM): 🔴 Comprehensive — dedicated Analytics route, status-breakdown chart, time-series trend chart (created vs. completed, day/week/month granularity), drill-down into the existing admin-search request panel, CSV export of aggregated data.
System Designer note (SDR): no new infrastructure justified — this is an internal, admin-only, low-traffic read over a small table (`pickup_requests`) that already has indexes on `status` and `created_at`. Plain Postgres `GROUP BY` aggregate queries are sufficient; no cache, queue, or OLAP store. Re-evaluate only if request volume grows by orders of magnitude.
Software Architect note (ADR): endpoints added to the existing `admin` package (layered/package-by-feature, matches Story 5.3's precedent) rather than a new module — analytics-over-requests is a read concern naturally owned by the admin package, not a new bounded context. "Completed" bucket in the time-series is approximated as `status = COMPLETED AND updated_at` in range (no dedicated `completed_at` column exists and requests don't get touched again after completion) — adding a new column was rejected as scope creep for what's a reporting nuance, not a business requirement; documented here so the approximation is explicit, not silent.
Drill-down implemented via router query params into the existing admin-search request panel (status/date pre-filled) rather than building a second results view — reuses Story 5.2/5.3's existing UI instead of duplicating it.
